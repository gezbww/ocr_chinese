...
def read_vocab(vocab_path,processor):
    """
    读取自定义训练字符集
    vocab_path format:
    1\n
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain_model', default='./cust-data/weights', type=str, help="预训练bert权重文件")

    parser.add_argument('--cust_data_init_weights_path', default='./cust-data/weights1', type=str,
    ...
    cust_vocab = read_vocab(args.cust_vocab,processor)
    print("-----------------------------")
    ...
    ##替换模型参数
    with open(os.path.join(args.cust_data_init_weights_path, "config.json"),encoding='utf-8') as f:
    ...
    with open(os.path.join(args.cust_data_init_weights_path, "config.json"), "w",encoding='utf-8') as f:
    ...
    print(pre_model_weigths)
    cust_model_weigths = cust_model.state_dict()
    print(cust_model_weigths)
