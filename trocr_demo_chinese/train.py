...
from transformers import VisionEncoderDecoderModel, ResNetModel
import pandas as pd
import torch
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_path', default='./checkpoint/trocr31', type=str, help="训练模型保存地址")
    parser.add_argument('--dataset_path', default='', type=str, help="训练数据集")
    ...
    parser.add_argument('--eval_steps', default=100000, type=int, help="模型评估间隔数")
    parser.add_argument('--save_steps', default=10, type=int, help="模型保存间隔步数")
    ...
    path1 = glob(args.dataset_path)
    print(len(path1))
    path2 = glob(args.datasets_path)
    ...
    train_dataset = trocrDataset(paths=path1, processor=processor, df=df,max_target_length=args.max_target_length, transformer=transformer)
    transformer = lambda x: x  ##图像数据增强函数
    eval_dataset = trocrDataset(paths=path2, processor=processor,df=df_test, max_target_length=args.max_target_length, transformer=transformer)
